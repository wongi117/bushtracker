...
class NaturalLanguageSearchScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<NaturalLanguageSearchScreen> createState() => _NaturalLanguageSearchScreenState();
}

class _NaturalLanguageSearchScreenState extends ConsumerState<NaturalLanguageSearchScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('🔍 AI Search', style: TextStyle(color: Colors.white)),
        leading: IconButton(
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.search, color: AppColors.textSecondary.withValues(alpha: 0.5), size: 64),
  ...
  void _performSearch(String query) {
    ...
    if (lowerQuery.contains('water') || lowerQuery.contains('dam') || lowerQuery.contains('river')) {
      final waterWaypoints = locationState.waypoints
          .where((w) => w.icon == 'water' || w.label?.toLowerCase().contains('water') == true)
          .toList();
      
      for (final wp in waterWaypoints) {
        if (wp.latitude != null && wp.longitude != null) {
          final dist = _calculateDistance(userLat, userLon, wp.latitude!, wp.longitude!);
    ...
    if (lowerQuery.contains('fuel') || lowerQuery.contains('petrol') || lowerQuery.contains('gas')) {
      final fuelWaypoints = locationState.waypoints
          .where((w) => w.icon == 'fuel' || w.label?.toLowerCase().contains('fuel') == true
              || w.label?.toLowerCase().contains('petrol') == true)
          .toList();

      for (final wp in fuelWaypoints) {
        if (wp.latitude != null && wp.longitude != null) {
          final dist = _calculateDistance(userLat, userLon, wp.latitude!, wp.longitude!);
          results.add(SearchResult(
            title: wp.label ?? 'Fuel Stop',
            subtitle: wp.notes ?? 'Fuel waypoint',
            icon: '⛽',
            distance: dist,
            coords: LatLng(wp.latitude!, wp.longitude!),
            isWaypoint: true,
      ...
      if (fuelWaypoints.isEmpty) {
        results.add(SearchResult(
          title: 'No fuel waypoints saved',
          subtitle: 'Drop a fuel waypoint on the map to save locations',
          icon: '⛽',
          isWaypoint: false,
        ));
      }
    ...
    if (lowerQuery.contains('camp') || lowerQuery.contains('campsite')) {
      final campWaypoints = locationState.waypoints
          .where((w) => w.icon == 'camp' || w.label?.toLowerCase().contains('camp') == true)
          .toList();
    ...
    if (lowerQuery.contains('where') || lowerQuery.contains('am i')) {
      results.add(SearchResult(
        ...
        subtitle: 'Lat: ${userLat.toStringAsFixed(4)}, Lon: ${userLon.toStringAsFixed(4)}',
        icon: '📍',
        distance: 0,
    ...
    if (results.isEmpty) {
      results.add(SearchResult(
        title: 'AI Suggestion',
        subtitle: 'Try: "nearest water", "fuel under 50km", "camp nearby", "how far to Perth"',
        icon: '💡',
        isWaypoint: false,
      ));
    }
  ...
  double _calculateDistance(double lat1, double lon1, double lat2, double lon2) {
    ...
    final a = 0.5 - _cos(lat2 * p) * _cos(lat1 * p) +
  ...
  double _cosTaylor(double x) => 1 - (x*x)/2 + (x*x*x*x)/24 - (x*x*x*x*x*x)/720;
  double _sinTaylor(double x) => x - (x*x*x)/6 + (x*x*x*x*x)/120;
